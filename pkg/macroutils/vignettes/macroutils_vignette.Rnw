...
% +~~~~~~~~~~~~~~~~~~~~~~~~~~~~+
\subsection{Plot variables from binary files (\texttt{macroPlotBin})} 
...
If you want to use \texttt{macroPlotBin} in a non-interactive way, 
...
macroPlotBin( x = tbl[, 1:3 ], gui = FALSE ) 
@ 
...
<<echo=TRUE,eval=TRUE,fig=TRUE>>= 
macroPlotBin( x = tbl[, 1:3 ], gui = FALSE, subPlot = FALSE ) 
@ 
